...
PackedByteArray ParticleGPUEncoder::encode_particle_ordered (Ref<ParticleBuffer> buffer,godot::Vector3 cameraPosition)
{
    ...
    auto dataContainer = buffer->get_particle_data_container();
    dataContainer->cull_particles(cameraPosition);
    auto data = buffer->get_particle_data_container();
    std::vector<Particle> ordered = dataContainer->get_particles();
...
godot::PackedByteArray ParticleGPUEncoder::encode_particle(Particle particle,Ref<ParticleBuffer> buffer)
{
    ...
    bytes.resize(32);
    bytes.encode_var(0,buffer->get_particle_data_container()->get_position(particle));
    Vector3 position = buffer->get_particle_data_container()->get_position(particle);
    bytes.encode_float(0,position.x);
    bytes.encode_float(4,position.y);
    bytes.encode_float(8,position.z);
    
    Color color =  buffer->get_particle_data_container()->get_particle_color(particle);
...
godot::PackedByteArray ParticleGPUEncoder::encode_particle_positions (godot::Ref<ParticleBuffer> buffer)
{
    ...
    for (auto &&particle : buffer->get_particle_data_container()->get_particles())
    {
...
godot::PackedByteArray ParticleGPUEncoder::encode_particle_colors (godot::Ref<ParticleBuffer> buffer)
{
    ...
    for (auto &&particle : buffer->get_particle_data_container()->get_particles())
    {
        ...
        Color color = buffer->get_particle_data_container()->get_particle_color(particle);
        bytes.encode_float(particle * 16, color.r);
        bytes.encode_float(particle * 16 + 4, color.g);
        bytes.encode_float(particle * 16 + 8, color.b);
...
godot::PackedByteArray ParticleGPUEncoder::encode_octree_particle_buffer (godot::Ref<ParticleBuffer> buffer)
{
    ...
    buffer->get_particle_data_container()->cull_particles();
    auto particle_buffer = buffer->get_particle_data_container()->get_particles();
